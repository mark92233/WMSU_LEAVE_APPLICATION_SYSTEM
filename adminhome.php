...
// --- 0. CRITICAL LOGOUT & AJAX ENDPOINT START (Must be first for clean output) ---
...
// Ensure includes are loaded for AJAX handler
require_once "dbRelated/db_connect.php";
require_once "dbRelated/operation.php";

$employeeManager = new EmployeeManager(); // Instantiate manager needed for AJAX search

// Handle Real-Time Employee Search AJAX Request
if (isset($_GET['action']) && $_GET['action'] === 'search_employees') {
    // This endpoint outputs JSON and stops execution immediately.
    
    $searchQuery = $_GET['search_query'] ?? '';
    $deptFilter = $_GET['dept_filter'] ?? '';
    
    // 1. Fetch data from the database using the search query
    $employees = $employeeManager->getAllEmployeesDetails($searchQuery);
    
    // 2. Perform PHP-side department filter (if needed, as DB query only handles search)
    if (!empty($deptFilter)) {
        $employees = array_values(array_filter($employees, function($emp) use ($deptFilter) {
            return ($emp['DepartmentID'] ?? null) == $deptFilter;
        }));
    }

    // 3. Output JSON and stop
    header('Content-Type: application/json');
    echo json_encode($employees);
    exit;
}
// --- CRITICAL LOGOUT & AJAX ENDPOINT END ---

session_start();

// --- 1. SETUP AND INCLUDES ---

// Includes needed for the rest of the controller logic
require_once "dbRelated/EmailSender.php";
require_once "dbRelated/LeavePdfGenerator.php";

// --- Core Utility Functions (Unmodified) ---

/**
 * Calculates the end date of a leave, excluding weekends (Saturday/Sunday).
 */
...
// --- 2. Initialize Manager Objects ---
$reqObj = new AdminRegistration();
// $employeeManager is already initialized at the top for AJAX
$creditManager = new LeaveCreditManager();
$applicationManager = new LeaveApplicationManager();
$approvalManager = new LeaveApprovalManager();
$depObj = new DepartmentManager();
$posObj = new PositionManager();
$notifyObj = new NotificationManager();
$emailSender = new EmailSender(); // Initialize here for use in POST handlers
...
// --- 3. Session Check & Header Data Setup ---
...
$allNotifs = $notifyObj->getNotifications();
$notifCount = count($allNotifs);

// Fetch the current user's profile data for the header/avatar
...
if (isset($_GET['msg'])) {
    switch ($_GET['msg']) {
        case 'activated': $actionMessage = "Account activated successfully."; break;
        case 'decision_saved': $actionMessage = "Decision submitted successfully."; break;
...
$displayLeaveList = ($Role === "Admin" || $Role === "Department Head" || $Role === "Dean") ? "flex" : "none";
$displayRequests = ($Role === "Admin") ? "flex" : "none";

$section = $_GET['section'] ?? ($Role === "Admin" ? "dashboard" : "apply_leave");

// --- 4. Handle POST Submissions ---

// Handle Account Activation (Admin Only)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['activate_id'])) {
    $targetID = trim($_POST['activate_id']);
    $userInfo = $reqObj->checkIdExists($targetID);
    
    if ($userInfo && $reqObj->activate($targetID)) {
        if (!empty($userInfo['Email'])) {
            $mailer = new EmailSender();
            $mailer->sendAccountConfirmation($userInfo['Email'], $targetID);
        }

        header("Location: adminhome.php?section=request&msg=activated");
        exit();
    } else {
        $actionMessage = "Activation failed. User ID may not exist.";
    }
}

// Handle Leave Approval (Admin Only)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['decision'], $_POST['leave_id'])) {
    $remarks = trim($_POST['remarks'] ?? '');
    $remarksToPass = empty($remarks) ? null : $remarks;
    $targetLeaveID = trim($_POST['leave_id']);
    $decision = trim($_POST['decision']);

    $success = $approvalManager->processApproval(
        $targetLeaveID,
        $employeeID,
        $decision,
        $remarksToPass
    ...
    if ($success) {
        $appDetails = $applicationManager->getOneApplicationWithApproval($targetLeaveID);
        
        if ($appDetails && !empty($appDetails['Email'])) {
            $pdfGenerator = new LeavePdfGenerator();
            $pdfContent = $pdfGenerator->generatePdfString($targetLeaveID);

            $emailSender->sendDecisionNotification(
                $appDetails['Email'],
        ...
        header("Location: adminhome.php?section=leave_list&msg=decision_saved");
        exit();
    } else {
        $actionMessage = "Failed submitting decision.";
    }
}

// Handle Apply Leave (User/Admin)
$inputs = [
    'type' => 0, 'start' => '', 'days' => 0,
...
if ($section === "apply_leave" && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        ...
        if ($fileMoveSuccess) {
            ...
            if ($success) {
                header("Location: adminhome.php?section=apply_leave&msg=submitted");
                exit();
            } else {
...
// --- 5. Data Pre-Fetching for Views ---
$leaveList = [];
$myLog = [];
$details = null;

if ($section === 'leave_list') {
...
<title><?= htmlspecialchars(ucfirst(str_replace('_', ' ', $section))); ?> | Admin Panel</title>
...
<style>
    ...
    /* =========================================
        ...
        @media (max-width: 768px) {
            ...
            .sidebar {
                ...
                /* CRITICAL FIX: Allow clicking through the empty space */
    ...
    <!-- Modal -->
    <style>
        /* --- COLOR VARIABLES (Scoped to this block) --- */
        :root {
            --primary-color: #b91c1c;    /* Deep Cardinal Red */
            --primary-hover: #991b1b;    /* Darker Red */
            --secondary-color: #450a0a;  /* Deep Merlot */
            --accent-bg: #fef2f2;        /* Pale Red Background */
            --text-dark: #1f2937;        /* Sharp Dark Grey */
            --text-light: #6b7280;       /* Muted Grey */
        }

        /* 1. The Row Hover Effect (for the table triggering the modal) */
        .emp-row {
            cursor: pointer;
        ...
        /* 2. The Modal Background (Centering & Blur) */
        .modal-overlay {
            display: none; /* Hidden by default */
            position: fixed;
            z-index: 10000; /* Extremely high Z-index to sit above sidebar */
            left: 0;
            top: 0;
            width: 100%;
            height: 100%;
            background-color: rgba(69, 10, 10, 0.5); /* Merlot tint */
        ...
        /* 3. The Profile Card (Modal Content) */
        .profile-card {
            background-color: #fff;
            width: 100%;
            max-width: 420px;
            ...
            /* Scroll logic for small screens/landscape */
            max-height: 90vh; 
            overflow-y: auto;
            display: flex;
            flex-direction: column;
        ...
        .card-header p { 
            margin: 5px 0 0; opacity: 0.9; font-size: 13px; font-weight: 300; letter-spacing: 0.5px; 
        }
        ...
        .card-body { 
            padding: 25px 30px; 
            background-color: #fff;
            overflow-y: auto; /* Allow body scroll if content is long */
        }
        ...
        @media (max-width: 480px) {
            .profile-card {
                width: 100%;
                height: auto;
                max-height: 85vh; /* slightly smaller on mobile to show overlay */
            }

            .card-header { padding: 25px 15px; }
            .avatar-container { width: 85px; height: 85px; }
            .card-header h2 { font-size: 20px; }

            .card-body { padding: 20px; }
            
            /* Stack split rows vertically on small screens */
            .row-split {
                flex-direction: column;
                gap: 0;
            }
...
<body>
    ...
    <div class="sidebar" id="mainSidebar">
        ...
        <form method="get" action="" class="sidebar-menu">
            ...
            <button type="submit" name="section" value="dashboard" class="<?= $section === 'dashboard' ? 'active' : '' ?>">
            ...
            <button type="submit" name="section" value="leave_list" class="<?= $section === 'leave_list' ? 'active' : '' ?>" style="display: <?= $displayLeaveList ?>;">
                <i class="fas fa-list"></i> <span>Leave List</span>
            </button>
            ...
            <button type="submit" name="section" value="request" class="<?= $section === 'request' ? 'active' : '' ?>" style="display: <?= $displayRequests ?>;">
                    ...
                    <div class="icon-button-wrapper">
                        ...
                        <div id="notificationDropdown" class="notif-dropdown">
                            <div class="notif-header">
                                <h3>Notifications</h3>
                                <span style="font-size:0.8rem; color:var(--text-muted);"><?= $notifCount ?> Pending</span>
                            </div>
                            <div class="notif-list">
                                <?php if (!empty($allNotifs)): ?>
                                    <?php foreach ($allNotifs as $notif):?>
                                        <a href="<?= $notif['link'] ?>" class="notif-item unread">
                                            <div class="notif-icon-box">
                                                <img src="<?= htmlspecialchars(!empty($notif['pic']) ? $notif['pic'] : 'assets/image/default-avatar.png') ?>" alt="User">
                                                <div class="notif-type-icon <?= $notif['bg_color'] ?> <?= $notif['color'] ?>">
                                            ...
                                            <div class="notif-content">
                                                ...
                                                <p style="font-size:0.75rem; color:#b91c1c; margin-top:2px;"><?= htmlspecialchars($notif['sub_text']) ?></p>
                            ...
                            <div style="padding:10px; text-align:center; border-top:1px solid #eee; background:#f9fafb; font-size:0.85rem; font-weight:600; color:var(--primary-color); cursor:pointer;">
                                View All Activity
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function confirmLogout() {
            // Targets the logout logic at the very top of this file
            window.location.href = "adminhome.php?action=logout";
        }
